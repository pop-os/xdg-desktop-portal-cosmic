...
pub(crate) fn has_tab(
    tab_model: &widget::segmented_button::Model<widget::segmented_button::SingleSelect>,
    tab: Tab,
) -> bool {
    tab_model
        .iter()
        .any(|entity| tab_model.data::<Tab>(entity) == Some(&tab))
}

/// Populate the source-picker tabs, shared between the screencast and
/// remote-desktop dialogs.
pub(crate) fn update_tab_model(
    tab_model: &mut widget::segmented_button::Model<widget::segmented_button::SingleSelect>,
    source_types: BitFlags<SourceType>,
    toplevels: &[(ToplevelInfo, Option<String>)],
) {
    tab_model.clear();
    let outputs = source_types.contains(SourceType::Monitor);
    let windows = source_types.contains(SourceType::Window) && (!toplevels.is_empty() || !outputs);
    if outputs {
        tab_model.insert().data(Tab::Outputs).text(fl!("output"));
    }
    if windows {
        tab_model.insert().data(Tab::Windows).text(fl!("window"));
    }
